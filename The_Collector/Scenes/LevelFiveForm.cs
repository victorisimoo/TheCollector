...
namespace The_Collector.Scenes {
    public partial class LevelFiveForm : Form {
        //Variables u objetos a utilizar en la clase.
        ...
        Soul badEight = new Soul();
        Soul badNine = new Soul();
        Soul badTen = new Soul();
        Soul badEleven = new Soul();
        Soul badTwelve = new Soul();
        private bool left;
        private bool right;
        private bool above;
        private bool down;
        ...
        public LevelFiveForm(Player player, List<Question> questions) {
            InitializeComponent();
            this.player = player;
        ...
        //Método para mover al usuario por el tablero.
        ...
        //Método encargado de verificar la interacción del usuario y los mostruos / almas.
        private void InteractionDetection_Tick(object sender, EventArgs e){
            if (((ptbPlayer.Left + ptbPlayer.Width) == (ptbSoulOne.Left + ptbSoulOne.Width)) && ((ptbPlayer.Location.Y + ptbPlayer.Height + 4) == (ptbSoulOne.Location.Y + ptbSoulOne.Height)) && ptbSoulOne.Visible != false){
                ...
                if (EvaluateRecolection() == true){
                    player.SetPointsPlayer(pointsUser + 50 + 1000);
                    moveUser.Enabled = false;
                    moveUser.Stop();
                    InteractionDetection.Enabled = false;
                    InteractionDetection.Stop();
                    this.Hide();
                    WinForm winnerForm = new WinForm(this.player);
            ...
            }else if (((ptbPlayer.Left + ptbPlayer.Width) == (ptbEight.Left + ptbEight.Width)) && ((player.GetCoordY() + ptbPlayer.Height) == (badEight.GetCoordY() + ptbEight.Height))){
                ptbEight.Visible = false;
                Reaction();

            }else if (((ptbPlayer.Left + ptbPlayer.Width) == (ptbNine.Left + ptbNine.Width)) && ((player.GetCoordY() + ptbPlayer.Height) == (badNine.GetCoordY() + ptbNine.Height))){
                ptbNine.Visible = false;
                Reaction();

            }else if (((ptbPlayer.Left + ptbPlayer.Width) == (ptbTen.Left + ptbTen.Width)) && ((player.GetCoordY() + ptbPlayer.Height) == (badTen.GetCoordY() + ptbTen.Height))){
                ptbTen.Visible = false;
                Reaction();

            }else if (((ptbPlayer.Left + ptbPlayer.Width) == (ptbEleven.Left + ptbEleven.Width)) && ((player.GetCoordY() + ptbPlayer.Height) == (badEleven.GetCoordY() + ptbEleven.Height))){
                ptbEleven.Visible = false;
                Reaction();

            }else if (((ptbPlayer.Left + ptbPlayer.Width) == (ptbTwelve.Left + ptbTwelve.Width)) && ((player.GetCoordY() + ptbPlayer.Height) == (badTwelve.GetCoordY() + ptbTwelve.Height))){
                ptbEight.Visible = false;
                Reaction();
            }
        }
        ...
        //Método para capturar el movimiento del usuario.
        ...
        //Método para capturar el paro de movimiento del usuario.
        ...
        //Método para llevar el control del tiempo del usuario.
